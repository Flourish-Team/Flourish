...
function m.deps(cfg)
    local deps = config.getlinks(cfg, "dependencies", "object")
    if deps and #deps>0 then
        for _, dep_config in ipairs(deps) do
            printf(cmake.targetname(dep_config))
            local targetname = cmake.targetname(cfg)
            _p(1, 'target_link_libraries(%s %s)', targetname, cmake.targetname(dep_config))
        end
    end
end
